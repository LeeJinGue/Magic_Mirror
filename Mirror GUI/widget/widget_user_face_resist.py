...
class resist(QWidget):
  ...
  def setUser(self):
    self.data = db_access.get_name_id()
    #print(self.data)
    
    temp_dic = {}
    for i in self.data:
      #print(i)
      temp_dic[i['name']] = i['user_num']
      self.adduser(i['name'])
    self.form.user_id = temp_dic
    #print(self.form.user_id)

  def name_list_click(self):
          s = self.name_view.currentItem()
          self.form.selected = s.text()
          self.label1.setText("선택된 사용자: "+ self.form.selected)
